...
Public Class ucPesajePalletsDespacho
    Enum PasoCalibrado
        Busqueda = 0
        Pesaje = 1
        Ubicacion = 2
    ...
    Public Sub GestionarPasos(paso As PasoCalibrado)
        ' 1. Apagamos todo
        UcValidacionPalletDespacho1.Visible = False ' Control de búsqueda de pallets
        pnlContenedorPesajePallets.Visible = False
        UcUbicacionPalletsDespacho1.Visible = False

        ' 2. Encendemos lo correspondiente
        Select Case paso
            Case PasoCalibrado.Busqueda
                UcValidacionPalletDespacho1.Visible = True
                UcValidacionPalletDespacho1.BringToFront()
            Case PasoCalibrado.Pesaje
                pnlContenedorPesajePallets.Visible = True
                pnlContenedorPesajePallets.BringToFront()
            Case PasoCalibrado.Ubicacion
                UcUbicacionPalletsDespacho1.Visible = True
                UcUbicacionPalletsDespacho1.BringToFront()
                UcUbicacionPalletsDespacho1.Dock = DockStyle.Fill
        End Select
    End Sub

    ' 🟢 RECEPCIÓN DE DATOS DEL PALLET (Sincronizado con los alias de tu consulta SQL)
    Private Sub ucValidacionPalletDespacho1_PalletValidado(pallets As List(Of DataRow)) Handles UcValidacionPalletDespacho1.PalletValidado
        CantidadTotalAProcesar = pallets.Count
        PasoActual = 1
        PesoAcumuladoAnterior = 0

        pnlContenedorPesajePallets.Controls.Clear()
        listaPesajes.Clear()
        ...
        For i As Integer = 1 To pallets.Count
            ...
            pnlContenedorPesajePallets.Controls.Add(nuevoContenedor)
    ...
    Private Sub AvanzarFlujo()
        ...
        Else
            ...
            UcUbicacionPalletsDespacho1.RecibirDatosParaGuardar(loteParaGuardar)
